...
func _test_automatic_load(game) -> void:
	...
	_assert_equal(game.heroes.size(), 10, "older saves receive newly authored roster heroes")
	_assert_equal(game.heroes[5]["id"], "maelis", "new class heroes append in roster order")
	_assert_equal(game.heroes[5]["level"], 1, "newly appended heroes start with default progression")
